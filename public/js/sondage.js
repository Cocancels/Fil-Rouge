...
$.ajax({
    method: "GET",
    url: "http://localhost/FilRouge/public/?page=sondage",
    dataType: "json",
    headers: {
...
function end() {
    ...
    window.location.href = "http://localhost/FilRouge/public/?page=sondage"
...
function time(sec) {
    ...
    setTimeout(() => {
        sec--
        if (sec === 0) {
            window.location.href = "http://localhost/FilRouge/public/?page=home"
        } else {
            $('.timer').html(sec)
            time(sec)
        }
